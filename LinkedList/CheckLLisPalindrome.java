public class CheckLLisPalindrome {
    ...
    public void reverse(){ //0(n)
        Node prev = null;
        Node curr = tail = head; //Assignment of things always happens from right to left in java
        Node next;

        while(curr != null){
            // 3 var aur 4 step reverse koo yaad rekhne kaa tarika hee yeeh yaad hee raklo abb
            next = curr.next;
            curr.next = prev;
            prev = curr;
            curr = next;
        }
        // curr null banjata loop kee baad so usko prev hee head hoga simple as that 
        head = prev;
    }

    public void deleteNthfromEnd(int n){
        //Calculate the size
        //Here sz stands for size
        int sz = 0;
        Node temp = head;
        while (temp != null) {
            temp = temp.next;
        ...
        if(n == sz){
            head = head.next; //remove first linkedlist
            return;
        }

        //baki sarre cases mein mujhe size-n tak puch naa hee okk lets go 
        // size - n node to delete seeh bss phele vala index hee 
        int i = 1;
        int iToFind = sz-n;
        Node prev = head;
        while(i < iToFind){
        ...
        //yaha seeh linking thod dii apen nhe okk lets go samja naa
        return;
    }

    //Slow Fast Approach
    public Node findMid(Node head){ //yeeh mera helper function hee okay 
        Node slow = head;
        Node fast = head;

        while(fast != null && fast.next!= null){
            slow = slow.next; //+1 hoga yeeh (Turtle)
            fast = fast.next.next; //+2 hoga yeeh  (Hare)
        }
    ...
    public boolean checkPalindrome(){
        //Base Case 
        if(head == null || head.next == null){ //null or head kaa next null raha toh base case okk
            return true;
        }
        // Step 1 - Find MidNode
        Node midNode = findMid(head);
        //Step 2 - 2 Half koo Reverse
        Node prev = null;
        Node curr = midNode;
        Node next;
        ...
        Node right = prev; //right half kaa head
        Node left = head;
